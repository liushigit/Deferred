...
extension FutureType {
    ...
    public func flatMap<NewFuture: FutureType>(upon queue: DispatchQueue, _ requestNextValue: (Value) -> NewFuture) -> Future<NewFuture.Value> {
        let d = Deferred<NewFuture.Value>()
        upon(queue) {
    ...
    /// - seealso: qos_class_t
    /// - seealso: FutureType.flatMap(upon:_:)
    public func flatMap<NewFuture: FutureType>(_ requestNextValue: (Value) -> NewFuture) -> Future<NewFuture.Value> {
        if let value = peek() {
