...
public struct DispatchLock: ReadWriteLock {
    ...
    private func withLock<Return>(timeout: Timeout, body: @noescape() throws -> Return) rethrows -> Return? {
        guard case .Success = semaphore.wait(timeout: .init(timeout)) else { return nil }
        defer {
            semaphore.signal()
        }
        return try body()
