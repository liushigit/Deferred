...
public protocol ReadWriteLock {
    ...
    func withReadLock<Return>(_ body: @noescape() throws -> Return) rethrows -> Return
    ...
    func withAttemptedReadLock<Return>(_ body: @noescape() throws -> Return) rethrows -> Return?
    ...
    /// - returns: The value returned from the given function.
    func withWriteLock<Return>(_ body: @noescape() throws -> Return) rethrows -> Return
}
...
extension ReadWriteLock {
    ...
    /// - parameter body: A function that writes a value while locked, then returns some value.
    /// - returns: The value returned from the given function.
    public func withWriteLock<Return>(_ body: @noescape() throws -> Return) rethrows -> Return {
...
public struct DispatchLock: ReadWriteLock {
    ...
    private func withLock<Return>(by timeout: DispatchTime, body: @noescape() throws -> Return) rethrows -> Return? {
        guard case .Success = semaphore.wait(timeout: timeout) else { return nil }
        defer {
            semaphore.signal()
        }
        return try body()
    ...
    /// - parameter body: A function that reads a value while locked.
    /// - returns: The value returned from the given function.
    public func withReadLock<Return>(_ body: @noescape() throws -> Return) rethrows -> Return {
        return try withLock(by: .distantFuture, body: body)!
    }

    /// Attempt to call `body` with a lock.
    /// - returns: The value returned from `body`, or `nil` if already locked.
    /// - seealso: withReadLock(_:)
    public func withAttemptedReadLock<Return>(_ body: @noescape() throws -> Return) rethrows -> Return? {
        return try withLock(by: .now(), body: body)
    }
}
