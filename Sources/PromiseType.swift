...
extension PromiseType {
    @available(*, unavailable, renamed: "preconditionFill")
    ...
    ///   configuration), and the deferred value is already filled, program
    ///   execution will be stopped a debuggable state.
    ///
    /// * In optimized builds (the default for a "Release" configuration), and
    ...
    ///   is treated as a serious programming error. The optimizer may assume
    ///   that it is not possible.
    @_transparent
    public func preconditionFill(_ value: Value) {
        if !fill(value) {
            preconditionFailure("Cannot fill an already-filled \(self.dynamicType)")
        }
    }
}
