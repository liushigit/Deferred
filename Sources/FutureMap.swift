...
extension FutureType {
    ...
    public func map<NewValue>(upon queue: DispatchQueue, _ transform: (Value) -> NewValue) -> Future<NewValue> {
        let d = Deferred<NewValue>()
        upon(queue) {
            d.fill(transform($0))
        }
    ...
    /// - seealso: qos_class_t
    /// - seealso: FutureType.map(upon:_:)
    public func map<NewValue>(_ transform: (Value) -> NewValue) -> Future<NewValue> {
