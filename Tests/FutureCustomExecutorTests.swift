...
private func delay<Value>(_ value: @autoclosure(escaping)(Void) -> Value, by interval: TimeInterval) -> Future<Value> {
    ...
    Deferred<Value>.genericQueue.after(when: .now() + interval) {
        d.fill(value())
    }
    return Future(d)
}
...
class FutureCustomExecutorTests: XCTestCase {
    ...
    func testUpon() {
        ...
        let expect = expectation(withDescription: "upon block called when deferred is filled")
