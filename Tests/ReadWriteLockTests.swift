...
class PerfTestThread: Thread {
    ...
    let joinLock = ConditionLock(condition: 0)
...
class ReadWriteLockTests: XCTestCase {
    ...
    func testMultipleConcurrentReaders() {
        for lock in locksAllowingConcurrentReads {
            ...
            for _ in 0 ..< 32 {
                let expectation = self.expectation(withDescription: "read \(lock)")
                queue.async {
                    lock.withReadLock {
    ...
    func testMultipleConcurrentWriters() {
        for lock in allLocks {
            ...
            for i in 0 ..< 5 {
                let expectation = self.expectation(withDescription: "write \(lock) #\(i)")
                queue.async {
                    lock.withWriteLock {
    ...
    func testSimultaneousReadersAndWriters() {
        for lock in allLocks {
            ...
            let startReader: (Int) -> () = { i in
                let expectation = self.expectation(withDescription: "reader \(i)")
                self.queue.async {
                    lock.withReadLock {
            ...
            let expectation = self.expectation(withDescription: "writer")
            queue.async {
                lock.withWriteLock {
                    for _ in 0 ..< 5 {
